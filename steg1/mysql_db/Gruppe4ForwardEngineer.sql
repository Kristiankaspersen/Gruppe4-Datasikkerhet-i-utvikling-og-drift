...
CREATE INDEX `fk_course_lecturer1_idx` ON `GruppeFireDB`.`course` (`lecturer_lecturer_id` ASC);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `GruppeFireDB`.`VWlecturer` (`id` INT);
...
AS 
SELECT  U.*, L.*, C.course_id, C.course_name
FROM  lecturer AS L 
...
JOIN course AS C
ON L.lecturer_id = C.lecturer_lecturer_id AND L.lecturer_id = LHU.lecturer_lecturer_id;
